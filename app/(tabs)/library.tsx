  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabs}>
        {TABS.map(tab => (
          <TouchableOpacity key={tab.value} style={[styles.tab, activeTab === tab.value && styles.tabActive]} onPress={() => setActiveTab(tab.value)}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        {loading ? (
          ...
          <TouchableOpacity key={i} style={styles.bookItem} onPress={() => router.push(`/book/${book.book_id}`)}>
            <View style={styles.bookCover}><Text style={{ fontSize: 24 }}>📚</Text></View>
              ...
              <View style={styles.tags}>
                {book.format && (
                  <View style={styles.formatPill}>
                    <Text style={styles.formatPillText}>{book.format === 'physical' ? '📖 Physique' : '📱 Liseuse'}</Text>
                  </View>
                )}
                {book.genres?.slice(0, 2).map((g: string, j: number) => (
                  <View key={j} style={styles.tag}><Text style={styles.tagText}>{g}</Text></View>
                ))}
              </View>
            </View>
            <View style={styles.bookRight}>
              {book.rating ? <Text style={styles.rating}>⭐ {book.rating}</Text> : null}
...
const makeStyles = (colors: ColorPalette) => StyleSheet.create({
  ...
  tabs: { flexDirection: 'row', backgroundColor: colors.card, borderRadius: radius.md, padding: 4, marginHorizontal: 16, marginBottom: 8 },
  ...
  bookItem: { flexDirection: 'row', gap: 12, padding: 12, backgroundColor: colors.card, borderRadius: radius.md, marginBottom: 8, alignItems: 'center', borderWidth: 1, borderColor: colors.divider },
  bookCover: { width: 42, height: 58, backgroundColor: colors.card2, borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
  bookInfo: { flex: 1 },
  ...
  formatPill: { borderWidth: 1, borderColor: colors.teal, paddingHorizontal: 7, paddingVertical: 2, borderRadius: 8 },
  formatPillText: { fontSize: 9, color: colors.teal },
  ...
  sheetTitle: { fontSize: 16, fontWeight: '700', color: colors.white, textAlign: 'center', marginBottom: 20 },
  sheetBtn: { padding: 14, backgroundColor: colors.card2, borderRadius: radius.md, marginBottom: 8, borderWidth: 1, borderColor: colors.divider },
  sheetBtnText: { color: colors.white, fontSize: 14, fontWeight: '500' },
  sheetBtnDanger: { borderColor: 'rgba(217,117,103,0.3)' },
