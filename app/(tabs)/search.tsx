  ...
  >
    {books.map((book, i) => (
      ...
      >
        ...
        <Text style={styles.hAuthor} numberOfLines={1}>
...
export default function SearchScreen() {
    ...
    userBooks
      ...
      .then((all) => {
        const excludeIds = new Set(
          all.map((b) => b.external_id).filter(Boolean),
        );
  ...
  const findOwned = (book: any): userBooks.UserBook | undefined => {
    ...
    return libraryBooks.find(
      (b) =>
        b.external_id === book.external_id ||
        userBooks.bookKey(b.title, b.author) === key,
    );
  };
  ...
  const loadTrending = () => {
    ...
    Promise.all([books.getTrending(), books.getPopular()])
  ...
  const openDetail = (book: any) => {
    ...
    if (!book.description) {
      books.getWorkDescription(book.external_id).then((description) => {
        if (description)
          setSelectedBook((cur: any) =>
            cur?.external_id === book.external_id
              ? { ...cur, description }
              : cur,
          );
      });
    ...
    books
      .getBookIdByExternalId(book.external_id)
      .then((bookId) => {
        if (!bookId) return;
        Promise.all([
          userBooks.getBookRatingStats(bookId),
          userBooks.getBookReviews(bookId),
        ])
          .then(([ratingStats, reviews]) => {
            setSelectedBook((cur: any) =>
              cur?.external_id === book.external_id
                ? { ...cur, ratingStats, reviews }
                : cur,
            );
          })
          .catch(() => {});
  ...
  const addBook = (book: any, status: string = "to_read") => {
    ...
    books
      ...
      .then((row) => {
        userBooks
          .addBookSmart(row.id, status, book.title, book.author)
          .then(() => {
            loadLibrary();
            setShowDetail(false);
            showSuccess(
              t(STATUS_TOAST_KEY[status] ?? "search.addedToRead", {
                title: book.title,
              }),
            );
          });
            ...
            {selectedBook && (
              <>
                <View style={styles.modalBook}>
                  <TouchableOpacity
                    onPress={() => openBookDetail(selectedBook)}
                  >
                    <View style={styles.modalCover}>
                      {selectedBook.cover_url ? (
                        <Image
                          source={{ uri: selectedBook.cover_url }}
                          style={styles.modalCoverImg}
                        />
                      ) : (
                        <Feather name="book" size={28} color={colors.purple} />
                      )}
                    </View>
                  </TouchableOpacity>
                  <View style={styles.modalInfo}>
                    ...
                    {selectedBook.genres?.length > 0 && (
                      <View style={styles.modalTags}>
                        {books
                          .normalizeTags(selectedBook.genres)
                          .map((g: string, i: number) => (
                            <Pill
                              key={i}
                              label={g}
                              onPress={() => searchByTag(g)}
                            />
                          ))}
                      </View>
                    )}
                ...
                selectedBook.reviews?.length > 0 ? (
                  ...
                  >
                    ...
                    {selectedBook.reviews?.length > 0 && (
                      <View
                        style={{ marginTop: selectedBook.description ? 16 : 0 }}
                      >
                        <Text style={styles.reviewsHeader}>
                          {t("search.reviewsHeader")}
                        </Text>
                        {selectedBook.reviews.map((r: any, i: number) => (
                          <View
                            ...
                            style={[
                              styles.reviewItem,
                              i < selectedBook.reviews.length - 1 &&
                ...
                <View style={{ gap: 8 }}>
                  {STATUS_OPTIONS.map((opt) => {
                    const current =
                      findOwned(selectedBook)?.status === opt.status;
                    return (
                      <Button
                        key={opt.status}
