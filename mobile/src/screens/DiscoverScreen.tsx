...
export default function DiscoverScreen({ navigation }: any) {
  ...
  useFocusEffect(useCallback(() => {
    booksService.getMyBooks('to_read').then(res => setRecentBooks(res.data.slice(0, 6))).catch(() => {});
    friendsService.getPendingRequests().then(res => setPendingCount(res.data.length)).catch(() => {});
  }, []));
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {recentBooks.length === 0 ? (
            ...
            <TouchableOpacity key={i} style={styles.miniCard}
              onPress={() => navigation.getParent()?.navigate('BookDetail', { book })} activeOpacity={0.75}>
...
const makeStyles = (colors: ColorPalette) => StyleSheet.create({
  ...
  chip: { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20, borderWidth: 1, borderColor: 'rgba(108,92,231,0.3)', marginRight: 8 },
  ...
  genreBadge: { alignSelf: 'flex-start', backgroundColor: 'rgba(0,206,201,0.1)', paddingHorizontal: 10, paddingVertical: 3, borderRadius: 20 },
  ...
  actBtn: { flex: 1, padding: 13, borderRadius: radius.md, borderWidth: 1, borderColor: 'rgba(108,92,231,0.3)', backgroundColor: colors.card, alignItems: 'center' },
