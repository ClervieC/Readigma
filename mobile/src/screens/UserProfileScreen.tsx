...
import { View, Text, ScrollView, StyleSheet, SafeAreaView, TouchableOpacity, Image, ActivityIndicator } from 'react-native';
...
export default function UserProfileScreen({ route, navigation }: any) {
  ...
  useEffect(() => {
    friendsService.getUserProfile(userId).then(res => setData(res.data)).catch(() => {}).finally(() => setLoading(false));
  }, [userId]);
          ...
          <View style={styles.section}>
            ...
            {currentlyReading.map((book: any, i: number) => (
              <View key={i} style={styles.bookCard}>
                <View style={styles.bookCover}><Text style={{ fontSize: 28 }}>📚</Text></View>
                <View style={styles.bookInfo}>
                  <Text style={styles.bookTitle} numberOfLines={2}>{book.title}</Text>
                  <Text style={styles.bookAuthor}>{book.author}</Text>
                  {book.genres?.[0] && (
                    <View style={styles.genreBadge}><Text style={styles.genreText}>{book.genres[0]}</Text></View>
                  )}
...
const makeStyles = (colors: ColorPalette) => StyleSheet.create({
  ...
  avatarPlaceholder: { width: 80, height: 80, borderRadius: 40, backgroundColor: colors.purple, alignItems: 'center', justifyContent: 'center' },
  ...
  statBox: { flex: 1, backgroundColor: colors.card, borderRadius: radius.md, padding: 12, alignItems: 'center', gap: 4, borderWidth: 1, borderColor: colors.divider },
  ...
  bookCard: { flexDirection: 'row', gap: 12, alignItems: 'flex-start', paddingBottom: 14, marginBottom: 14, borderBottomWidth: 1, borderBottomColor: colors.divider },
  bookCover: { width: 56, height: 76, backgroundColor: colors.card2, borderRadius: 8, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  bookInfo: { flex: 1 },
  ...
  genreBadge: { alignSelf: 'flex-start', backgroundColor: 'rgba(34,211,238,0.1)', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 20, marginBottom: 8 },
  ...
  emptyReading: { backgroundColor: colors.card, borderRadius: radius.md, padding: 20, alignItems: 'center', borderWidth: 1, borderColor: colors.divider, marginBottom: 16 },
