...
export default function LibraryScreen({ navigation }: any) {
  ...
  const loadBooks = () => {
    setLoading(true);
    booksService.getMyBooks().then(res => { setAllBooks(res.data); setLoading(false); }).catch(() => setLoading(false));
  };
  ...
  const changeStatus = (status: string) => {
    booksService.updateBook(selectedBook.book_id, { status }).then(() => { setSelectedBook(null); loadBooks(); });
  };
  ...
  const removeBook = () => {
    Alert.alert('Retirer', 'Retirer ce livre de ta liste ?', [
      ...
      { text: 'Retirer', style: 'destructive', onPress: () => {
        booksService.removeBook(selectedBook.book_id).then(() => { setSelectedBook(null); loadBooks(); });
      }}
    ]);
  };
