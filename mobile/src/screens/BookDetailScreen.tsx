...
function StarRating({ rating, onChange }: { rating: number; onChange: (r: number) => void }) {
  ...
  return (
    <View style={starStyles.container}>
      {[1, 2, 3, 4, 5].map(star => (
        <View key={star} style={starStyles.starWrap}>
          {steps.map(step => (
            <TouchableOpacity
              key={step}
              style={[starStyles.segment, { width: `${step * 100}%` as any }]}
              onPress={() => onChange(star - 1 + step)}
            />
          ))}
          <Text style={[starStyles.star, { color: rating >= star ? colors.purple : rating >= star - 0.5 ? colors.lavender : colors.gray }]}>
            {rating >= star ? '★' : rating >= star - 0.5 ? '⯨' : '☆'}
          </Text>
        </View>
      ))}
      <Text style={starStyles.value}>{rating > 0 ? rating.toFixed(2) : '—'}</Text>
    </View>
  );
}
...
const starStyles = StyleSheet.create({
  ...
  starWrap: { width: 36, height: 36, position: 'relative', alignItems: 'center', justifyContent: 'center' },
  segment: { position: 'absolute', left: 0, top: 0, bottom: 0, zIndex: 1 },
  star: { fontSize: 28, zIndex: 0 },
  value: { fontSize: 14, color: colors.gray, marginLeft: 8 },
});
...
export default function BookDetailScreen({ route, navigation }: any) {
  ...
  const updateProgress = () => {
    ...
    setLoading(true);
    booksService.updateProgress(book.book_id, {
      ...
      setLoading(false);
      Alert.alert('✅', 'Progression mise à jour !');
  ...
  const changeStatus = (status: string) => {
    ...
    booksService.addBook(book.book_id, 'done').then(() =>
      booksService.updateBook(book.book_id, { status: 'done', rating: rating || undefined, comment: comment || undefined })
        ...
        console.log('Error finishing book:', err.response?.data, err.message);
  ...
  const addReaction = () => {
    ...
    booksService.addReaction(book.book_id, {
      emoji: selectedEmoji, note: reactionNote,
      progress_percent: progress,
      page_number: parseInt(currentPage) || undefined,
      is_public: isPublic,
    }).then(() => {
      setShowReactionModal(false);
      setSelectedEmoji(''); setReactionNote('');
      loadReactions();
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.bookInfo}>
            ...
            {currentBook.genres?.[0] && (
              <View style={styles.genreBadge}>
                <Text style={styles.genreText}>{currentBook.genres[0]}</Text>
              </View>
            )}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.statusGrid}>
            {STATUS_OPTIONS.map(s => (
              <TouchableOpacity
                key={s.value}
                style={[styles.statusBtn, currentBook.status === s.value && styles.statusBtnActive]}
                onPress={() => changeStatus(s.value)}
              >
                <Text style={[styles.statusText, currentBook.status === s.value && styles.statusTextActive]}>
                  {s.label}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        {currentBook.status === 'reading' && (
          <View style={styles.section}>
            ...
            <View style={styles.modeToggle}>
              <TouchableOpacity
                style={[styles.modeBtn, progressMode === 'pages' && styles.modeBtnActive]}
                onPress={() => setProgressMode('pages')}
              >
                <Text style={[styles.modeBtnText, progressMode === 'pages' && styles.modeBtnTextActive]}>Par pages</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.modeBtn, progressMode === 'percent' && styles.modeBtnActive]}
                onPress={() => setProgressMode('percent')}
              >
                <Text style={[styles.modeBtnText, progressMode === 'percent' && styles.modeBtnTextActive]}>Par %</Text>
              </TouchableOpacity>
            </View>
            ...
            {progressMode === 'pages' ? (
              <View style={styles.pagesRow}>
                <View style={styles.pageInput}>
                  ...
                  <TextInput style={styles.input} value={currentPage} onChangeText={setCurrentPage}
                    keyboardType="number-pad" placeholder="0" placeholderTextColor={colors.gray} />
                </View>
                <Text style={styles.slash}>/</Text>
                <View style={styles.pageInput}>
                  <Text style={styles.inputLabel}>Total pages</Text>
                  <TextInput style={styles.input} value={totalPages} onChangeText={setTotalPages}
                    keyboardType="number-pad" placeholder="0" placeholderTextColor={colors.gray} />
                </View>
              </View>
            ) : (
              <View style={styles.percentRow}>
                ...
                <View style={styles.percentInputRow}>
                  <TextInput style={[styles.input, { flex: 1 }]} value={progressPercent}
                    onChangeText={setProgressPercent} keyboardType="decimal-pad"
        ...
        {currentBook.status === 'done' && (
          <View style={styles.section}>
            ...
            <TextInput
              style={[styles.input, { marginTop: 12, textAlign: 'left', height: 80 }]}
              value={comment} onChangeText={setComment}
              placeholder="Mon avis sur ce livre... (optionnel)"
              placeholderTextColor={colors.gray} multiline
            />
            <TouchableOpacity style={[styles.updateBtn, { marginTop: 12 }]}
              onPress={() => {
                booksService.updateBook(book.book_id, { rating, comment }).then(() =>
                  ...
                  <View style={styles.timelineContent}>
                    <View style={styles.timelineHeader}>
                      ...
                      <View style={styles.timelineMeta}>
                        ...
                        <Text style={styles.timelinePublic}>{r.is_public ? '🌍' : '🔒'}</Text>
                      </View>
                    </View>
      ...
      <Modal visible={showReactionModal} transparent animationType="slide">
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowReactionModal(false)}>
          <TouchableOpacity style={styles.modalSheet} activeOpacity={1}>
            ...
            <TextInput style={styles.noteInput} value={reactionNote} onChangeText={setReactionNote}
              placeholder="Ajoute une note..." placeholderTextColor={colors.gray} multiline maxLength={200} />
      ...
      <Modal visible={showFinishModal} transparent animationType="slide">
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowFinishModal(false)}>
          <TouchableOpacity style={styles.modalSheet} activeOpacity={1}>
            ...
            <StarRating rating={rating} onChange={setRating} />
            <TextInput style={[styles.noteInput, { marginTop: 16 }]}
              value={comment} onChangeText={setComment}
              placeholder="Ton avis sur ce livre... (optionnel)"
              placeholderTextColor={colors.gray} multiline maxLength={500} />
...
const styles = StyleSheet.create({
  ...
  genreBadge: {
    alignSelf: 'flex-start', backgroundColor: 'rgba(0,206,201,0.1)',
  ...
  section: {
    backgroundColor: colors.card, borderRadius: radius.lg,
    padding: 16, marginTop: 12, borderWidth: 1, borderColor: colors.divider,
  },
  ...
  statusGrid: { gap: 8 },
  statusBtn: {
    padding: 12, backgroundColor: colors.card2,
  ...
  modeToggle: {
    ...
    borderRadius: radius.sm, padding: 3, marginBottom: 14,
  },
  ...
  input: {
    ...
    padding: 12, color: colors.white, fontSize: 16,
  ...
  percentInputRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 4 },
  ...
  finishBtn: {
    flex: 1, backgroundColor: 'rgba(0,206,201,0.15)',
    borderRadius: radius.md, padding: 14, alignItems: 'center',
    borderWidth: 1, borderColor: colors.teal,
  },
  finishBtnText: { color: colors.teal, fontSize: 13, fontWeight: '700' },
  addReactionBtn: {
    backgroundColor: 'rgba(108,92,231,0.2)', paddingHorizontal: 12, paddingVertical: 6,
    borderRadius: 20, borderWidth: 1, borderColor: 'rgba(108,92,231,0.4)',
  },
  ...
  timelineDot: {
    ...
    backgroundColor: colors.purple, borderWidth: 2, borderColor: colors.lavender,
  },
  ...
  emojiBtn: {
    width: 48, height: 48, borderRadius: 12, backgroundColor: colors.card2,
  ...
  noteInput: {
    backgroundColor: colors.card2, borderRadius: radius.sm, padding: 12,
    color: colors.white, fontSize: 14, borderWidth: 1, borderColor: colors.divider,
    minHeight: 80, marginBottom: 12,
  },
  publicToggle: {
    padding: 12, backgroundColor: colors.card2, borderRadius: radius.sm,
    marginBottom: 12, alignItems: 'center', borderWidth: 1, borderColor: colors.divider,
  },
